...
>(({ ...props }, forwardRef) => {
    ...
    const renderPrefix = useCallback(() => {
        return (
            <TooltipWrapper className="text-body-14B" placement="top">
                <Button className="border-b border-dashed border-grey-1">{t('claim_price')}</Button>
            </TooltipWrapper>
        );
    }, []);
    ...
    return (
        <>
            <InputNumber ref={forwardRef} prefix={renderPrefix()} suffix={renderSuffix()} {...props} descriptionMessage={t('claim_price')} />

            <AdjustClaimPrice toggle={toggle} handleToggle={() => setToggle(status => !status)} descriptionMessage={t('claim_price')} {...props} />
        </>
    );
});

export default ClaimPriceInput;