...
import { UMBRELLAZ_KEY } from './utils/constant';
...
export function middleware(req: NextRequest) {
  const symbol = req.cookies[UMBRELLAZ_KEY.SYMBOL] ?? 'BNBUSDT';
  if (
  ...
  if (req.nextUrl.pathname.endsWith('/buy-cover')) {
    return NextResponse.redirect(new URL(`${req.nextUrl.pathname}/${symbol}`, req.url));
  }
